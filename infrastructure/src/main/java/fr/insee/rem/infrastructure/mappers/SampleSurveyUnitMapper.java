...
@Mapper(uses = {
    SampleMapper.class, SurveyUnitMapper.class
})
public interface SampleSurveyUnitMapper {
    ...
    @Mapping(
        target = "id",
        expression = "java(new fr.insee.rem.infrastructure.entity.SampleSurveyUnitPK(sampleSurveyUnitDto.getSample().getId(), sampleSurveyUnitDto.getSurveyUnit().getRepositoryId()))")
