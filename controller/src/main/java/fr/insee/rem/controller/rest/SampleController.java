...
public class SampleController {
    ...
    public ResponseEntity<SampleDto> createSample(@RequestBody String label) {
    ...
    @Operation(summary = "Get sample", responses = {
            @ApiResponse(responseCode = "200", description = "Sample successfully recovered"),
            @ApiResponse(responseCode = "404", description = "Sample Not Found")
    })
    @GetMapping(path = "/{sampleId}")
    public ResponseEntity<SampleDto> getSample(@PathVariable("sampleId") final Long sampleId) {
    ...
    public ResponseEntity<Object> deleteSample(@PathVariable("sampleId") final Long sampleId) {
        ...
        log.info("DELETE /samples/{sampleId} resulting in {} with response [{}]", response.getHttpStatus(), response
            .getMessage());
